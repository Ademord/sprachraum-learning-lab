...
for (const file of ['lessons.js','advanced-lessons.js','lesson-format.js','session-format.js','lesson-library.js','lesson-authoring.js','room-client.js','collaboration.js','app.js']) run(fs.readFileSync('dist/' + file, 'utf8'));
...
const range = w.document.createRange(); range.setStart(container.firstChild, start); range.setEnd(container.firstChild, start + 5);
...
assert(w.document.querySelector('.selection-action')); w.document.querySelector('.selection-action').click();
assert.equal(run('state.annotations.length'), 1); assert.equal(w.document.querySelectorAll('mark').length, 1);
...
run("closeDrawer();state.presentation.role='learner';render()"); assert.equal(w.document.querySelectorAll('mark').length, 0);
w.document.querySelector('[data-show-marks]').click(); assert.equal(w.document.querySelectorAll('mark').length, 1);
w.document.querySelector('[data-show-marks]').click(); assert.equal(w.document.querySelectorAll('mark').length, 0);
