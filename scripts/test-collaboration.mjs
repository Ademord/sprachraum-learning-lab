...
const notebookPanel=hostDocument.querySelector('.drawer'), notebookBackdrop=hostDocument.querySelector('.drawer-backdrop'), notebookNavigation=hostDocument.querySelector('.notebook-sections');
...
assert.equal(hostDocument.querySelector('.drawer'),notebookPanel,'section changes keep the open panel instead of replaying its entrance');
assert.equal(hostDocument.querySelector('.drawer-backdrop'),notebookBackdrop);
assert.equal(hostDocument.querySelector('.notebook-sections'),notebookNavigation);
assert.equal(hostDocument.activeElement.dataset.notebookSection,'live');
const createButton=hostDocument.querySelector('[data-create-live]');
hostDocument.querySelector('[data-notebook-section="live"]').click();
assert.equal(hostDocument.querySelector('[data-create-live]'),createButton,'clicking the active section does not rebuild its content');
hostDocument.querySelector('[data-notebook-section="notes"]').click();
assert.equal(hostDOM.run('drawer'),'notes');
assert.equal(hostDocument.querySelector('.drawer'),notebookPanel);
hostDOM.run("savePhrase('Saved while Notebook is open');openDrawer('notes')");
assert(hostDocument.querySelector('[data-drawer-content]').textContent.includes('Saved while Notebook is open'),'same-section programmatic refresh still updates the contents');
assert.equal(hostDocument.querySelector('.drawer'),notebookPanel);
