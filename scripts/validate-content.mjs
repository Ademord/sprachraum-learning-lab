...
const context={document:{querySelector:element,querySelectorAll:()=>[],addEventListener(){},body:{style:{}}},localStorage:{getItem:k=>stored.get(k),setItem:(k,v)=>stored.set(k,v)},setTimeout:()=>0,clearTimeout(){},addEventListener(){},console};context.window=context;vm.createContext(context);
for(const file of ['lessons.js','advanced-lessons.js','app.js'])vm.runInContext(fs.readFileSync(`dist/${file}`,'utf8'),context,{filename:file});
