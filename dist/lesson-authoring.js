...
async function readLessonFile(event) {
  ...
  if (file.size > LESSON_IO.maxBytes) {
    importResult = { ok: false, repairable: false, errors: ['File: choose a lesson JSON file smaller than 1 MB.'], warnings: [] }; paintImportResult(); event.target.value = ''; return;
  }
  try {
...
function checkImport() { importResult = LESSON_IO.validate(importText); paintImportResult(); }
...
function installImportedLesson(input) {
  const added = LESSON_LIBRARY.add(input);
